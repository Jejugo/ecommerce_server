...
const makeRegisterCustomer = ({
  ...
  const saveToStripe = (customer) =>
    stripeMethods.createUser({
      name: customer.getName(),
      email: customer.getEmail(),
    });

  const saveToDb = (customer, hashedPassword, stripeId) =>
    Customer.create(
      builder.customerRegistration(customer, hashedPassword, stripeId)
    ).catch((err) => {
  ...
  const generateEmailToken = (email) =>
    jwt.sign(email, process.env.EMAIL_TOKEN);
